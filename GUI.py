...
class GUI:
    ...
    def layout(self, gui):
        with ui.row().classes("w-full items-center"):
            ...
            ui.select(
                [1, 5, 10], value=1, on_change=lambda e: self.update_fan_config(e.value)
            )
            ui.label("LED Brightness:").classes("w-1/10 text-lg")
            ui.slider(min=0, max=100, value=100).classes("w-1/8").on(
                "update:model-value",
                lambda e: gui.update_led_config(e.args),
                throttle=1.0,
            )
            ui.button("Turn on Fans", on_click=lambda: print("Turning")).classes(
                "w-1/8"
            )
            ui.button("Turn off Fans", on_click=lambda: print("Turning")).classes(
                "w-1/8"
            )
